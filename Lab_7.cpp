...
int main()
{
	...
	Неявное приведение типов - приведение типов, выполняемое компилятором автоматически, в зависимости от контекста.К примеру,
	при передаче числа int в фукнцию, которая принимает double, компилятор определяет, что необходимо сменить тип аргумента с int на double, 
	при этом выводит предупреждение(warning) о возможной потере точности.
	...
	*/
	...
	// При работе с константами (типы переменных не указываются, т.к. переменных нет) принудительно задать число как дробное можно, 
	поставив точку и дробную часть после точки:
	...
	// Функции округления round() - стандартное округление к ближайшему целому, floor() - округление к меньшему, ceil() - округление к 
	большему целому из библиотеки <math.h> в Си или <cmath> при использовании компилятора C++.
	...
	*/
	...
	Задания
	Воспроизведите с комментариями в коде примеры из данного методического руководства в новом проекте Visual Studio (в составе единого solution). 
	Сделайте резервную копию результатов в локальном и удалённом репозитории git.

	На усмотрение учащихся для более уверенного владения языком C++ сверх программы рекомендуется освоить специальные 
	разновидности приведения типов: reinterpret_cast, dynamic_cast, const_cast.
	...
	*/
	...
	Файловая система - способ организации хранения, доступа, удаления, изменения, именования данных на носителях информации (как правило, 
	энергонезависимых - прим. авт.).
	...
	Все операции по работе с файлами (и с другими общими ресурсами машины: порты, сетевые сокеты, драйверы оборудования, 
	нераспределённая оперативная память) выполняются не напрямую программной, и даже не библиотечными функциями, а при посредничестве ОС. 
	Т.к. к примеру функция fwrite не пишет на сектора жёсткого диска напрямую, а обращается к соответствующей функции ОС, которая производит 
	...
	* Если указывать имя файла без абсолютного пути, то файл будет открываться из текущего каталога
	...
	при запуске программы без Visual Studio (по двойному клику в проводнике Windows) текущим каталогом является тот же каталоге, 
	где лежит программа;

	при запуске программы из терминала (cmd или PowerShell, или bash в Linux) текущим каталогом считается тот, который пишется на 
	каждой строчке в самом терминале перед командой.
	
	Функции и примеры Си
	
	Работа с файлами в Си происходит с помощью функций и типов данных стандартной библиотеки языка <stdio.h> (либо <cstdio> при 
	использовании фукций Си с компилятором C++). Логика работы с файлами в Си следующая:
	...
	только для чтения (для большей безопасности, если изменять его не предполагается или это может привести к сбоям). При этом функции 
	для записи работать не будут.
	
	только для записи (функции для чтения работать не будут).
	
	для чтения и записи.
	
	с сохранением содержимого файла и дозаписью в конец (файл открывается для записи или чтения/записи, и при открытии
	текущая позиция просто вставляется в конец файла).
	
	с перезаписью содержимого файла (при открытии в режиме записи или чтения/записи).
	
	Дальнейшие действия с файлом привязаны к текущей позиции в файле (почти как курсор в тексте). При чтении/записи 
	порции информации текущая позиция сдвигается в конец прочитанного/записанного фрагмента.
	
	При необходимости - сместить текущую позицию (фукнция fseek, fsetpos см. ниже)
	
	Закрыть файл (фактически, сообщить ОС, что работа с файлом до очередного открытия производиться не будет, и можно 
	позволить работать с данным файлом другим программам).
